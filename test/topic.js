...
describe('AliMNS-topic', () => {
  ...
  describe('Topic', function() {
    ...
    it('#setAttrsP & #getAttrsP', done => {
      ...
      topic
        .setAttrsP({ MaximumMessageSize: testSource })
        .then(dataSet => {
    ...
    it('Subscription #setAttrsP & #getAttrsP', done => {
      subscription
        .setAttrsP({ NotifyStrategy: AliMNS.Subscription.NotifyStrategy.EXPONENTIAL_DECAY_RETRY })
        .then(dataSet => {
          // console.info(dataSet);
    ...
    it('#publishP', done => {
      topic.publishP('Hello', false, null, null, { forever: true }).then(data => {
        // console.info(data);
        done();
      }, done);
    });
    ...
    it('#unsubscribe', done => {
      topic.unsubscribeP(subName).then(() => {
        done();
      }, done);
    });

    it('#deleteTopicP', done => {
      mns.deleteTopicP(topicName).then(() => {
        done();
      }, done);
    });
  });
  ...
  describe('Topic-Notify', function() {
    ...
    it('prepare-create-topic', done => {
      mns.createTopicP(topicName).then(() => {
        done();
      }, done);
    });
